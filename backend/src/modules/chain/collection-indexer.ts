...
async function resolveDeploymentBlock(latestBlock: bigint) {
  ...
  const latestCode = await arcClient.getBytecode({ address: maskBornAddress, blockNumber: latestBlock });
  ...
  while (low < high) {
    ...
    const code = await arcClient.getBytecode({ address: maskBornAddress, blockNumber: middle });
...
export async function syncCollectionTransfers(maxBatches = 25) {
  ...
  try {
    while (nextBlock <= latestBlock && batches < maxBatches) {
      ...
      const checkpoint = await arcClient.getBlock({ blockNumber: toBlock });
